...
Write-Host "Built $BuildPath/CasioEmuCore.js"

if ($CopyTo) {
    New-Item -ItemType Directory -Force $CopyTo | Out-Null
    Copy-Item (Join-Path $BuildPath "CasioEmuCore.js") $CopyTo -Force
    Copy-Item (Join-Path $BuildPath "CasioEmuCore.wasm") $CopyTo -Force
    if (Test-Path (Join-Path $BuildPath "CasioEmuCore.data")) {
        Copy-Item (Join-Path $BuildPath "CasioEmuCore.data") $CopyTo -Force
    }
    Write-Host "Copied core artifacts to $CopyTo"
}

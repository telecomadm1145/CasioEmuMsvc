...
struct MemoryEditor
{
    ...
    void DrawWindow(casioemu::MMU* mmu,const char* title, void* mem_data, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        Sizes s;
        ...
        if (ImGui::Begin(title, &Open, ImGuiWindowFlags_NoScrollbar))
        {
            ...
            DrawContents(mmu,mem_data, mem_size, base_display_addr);
    ...
    void DrawContents(casioemu::MMU* mmu,void* mem_data_void, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        if (Cols < 1)
            Cols = 1;

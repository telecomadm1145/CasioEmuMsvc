...
struct MemoryEditor
{
	...
	void DrawWindow(const char* title, void* mem_data, size_t mem_size, size_t base_display_addr, const OptionalMarkedSpans& marked_spans)
	{
		Sizes s;
		CalcSizes(s, mem_size, base_display_addr);
	...
	void DrawContents(void* mem_data_void, size_t mem_size, size_t base_display_addr, const OptionalMarkedSpans& marked_spans)
	{
		if (Cols < 1)
			Cols = 1;
						...
						if (data_write) {
							for (size_t i = 0; ; ++i) {
								...
								if (WriteFn) {
									...
									mem_data[addr + i] = (uint8_t)input_value;
								}
							}
						}
						ImGui::PopID();
	...
	void DrawOptionsLine(const Sizes& s, void* mem_data, size_t mem_size, size_t base_display_addr)
	{
		...
		if (ImGui::BeginPopup("context"))
		{
			...
			if (ImGui::DragInt("##cols", &Cols, 0.2f, 4, 128, "%d 列(拖动以改变)")) { ContentsWidthChanged = true; if (Cols < 1) Cols = 1; }
			ImGui::Checkbox("显示数据预览", &OptShowDataPreview);
			ImGui::Checkbox("混合显示ASCII和十六进制", &OptShowHexII);
			if (ImGui::Checkbox("显示 ASCII 字符", &OptShowAscii)) { ContentsWidthChanged = true; }

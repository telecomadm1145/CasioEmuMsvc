...
void WatchWindow::PrepareRX() {
	for (int i = 0; i < 16; i++) {
		sprintf((char*)reg_rx[i], "%02x", m_emu
			->chipset.cpu.reg_r[i] & 0x0ff);
	}
	sprintf(reg_pc, "%04x", m_emu
		->chipset.cpu.reg_pc & 0xffff);
	sprintf(reg_lr, "%04x", m_emu
		->chipset.cpu.reg_lr & 0xffff);
	sprintf(reg_sp, "%04x", m_emu
		->chipset.cpu.reg_sp & 0xffff);
	sprintf(reg_ea, "%04x", m_emu
		->chipset.cpu.reg_ea & 0xffff);
	sprintf(reg_psw, "%02x", m_emu

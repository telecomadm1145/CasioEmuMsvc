...
public:
	...
	void setPCL(uint8_t value) {
		pc = (pc & 0xff80) | (value & 0x7f);
	}

	void setPCM(uint8_t value) {
		pc = (pc & 0x807f) | ((value & 0x7f) << 7);
	}

	void setPCH(uint8_t value) {
		pc = (pc & 0x7fff) | ((value & 1) << 15);
	}

	void setTABPTRL(uint8_t value) {
		tabptr = (tabptr & 0xffffff00) | value;
	}

	void setTABPTRM(uint8_t value) {
		tabptr = (tabptr & 0xffff80ff) | ((value & 0x7f) << 8);
	}

	void setTABPTRH(uint8_t value) {
		tabptr = (tabptr & 0xfffeffff) | ((value & 1) << 16);
	}
	...
	uint16_t FetchInst() {
		return _byteswap_ushort(((uint16_t*)rom)[pc++]);
	}
	// --- Memory Access Helpers (implementing Sleigh readDat/writeDat logic) ---
	uint16_t FetchWord() {
		return _byteswap_ushort(((uint16_t*)rom)[pc++]);
	...
	void ReadDat(uint8_t rptr_arg) {
		...
		if (current_ptr_val < 0x80) {
			...
			if (effective_addr < sizeof(ram)) {
				rdata = ram[effective_addr];
			}
			else {
				rdata = 0xFF;
				std::cerr << "Warning: ReadDat out of RAM bounds: 0x" << std::hex << effective_addr << std::endl;
			}
		}

		if (is_indirect) {
			if (original_rptr_for_post_id == SFRs::INDF0) {
				if (getFSR0_PE()) {
					uint8_t temp_fsr0 = static_cast<uint8_t>(regs[SFRs::FSR0]);
					if (getFSR0_ID())
						regs[SFRs::FSR0] = static_cast<char>(temp_fsr0 + 1);
					else
						regs[SFRs::FSR0] = static_cast<char>(temp_fsr0 - 1);
				}
			}
			else if (original_rptr_for_post_id == SFRs::INDF1) {
				if (getFSR1_PE()) {
					uint8_t temp_fsr1 = static_cast<uint8_t>(regs[SFRs::FSR1]);
					uint8_t fsr1_masked = temp_fsr1 & 0x7F;
					if (getFSR1_ID())
						regs[SFRs::FSR1] = static_cast<char>(((fsr1_masked + 1) & 0x7F) | 0x80);
					else
						regs[SFRs::FSR1] = static_cast<char>(((fsr1_masked - 1) & 0x7F) | 0x80);
				}
			}
			else if (original_rptr_for_post_id == SFRs::INDF2) {
				if (getFSR2_PE()) {
					uint8_t temp_fsr2 = static_cast<uint8_t>(regs[SFRs::FSR2]);
					uint8_t fsr2_masked = temp_fsr2 & 0x7F;
					if (getFSR2_ID())
						regs[SFRs::FSR2] = static_cast<char>(((fsr2_masked + 1) & 0x7F) | 0x80);
					else
						regs[SFRs::FSR2] = static_cast<char>(((fsr2_masked - 1) & 0x7F) | 0x80);
				}
			}
	...
	void WriteDat(uint8_t rptr_arg, uint8_t value_to_write) {
		...
		if (current_ptr_val == SFRs::ZERO_REG) {
			...
			// 我们应该忽略对于 PCL PCM PCH
			// 以及 TBPTRL TBPTRM TBPTRH
			// 的写入，针对他们的 ADD 和 ADC 指令需要特殊处理
			// 以保证进位和退位
			if (current_ptr_val == SFRs::PCL || current_ptr_val == SFRs::PCM || current_ptr_val == SFRs::PCH ||
				current_ptr_val == SFRs::TABPTRL || current_ptr_val == SFRs::TABPTRM || current_ptr_val == SFRs::TABPTRH) {
				std::cout << "WARN: PC and TABPTR write ignored!\n";
			}
			if (current_ptr_val == SFRs::LCDDATA) {
				std::cout << "WARN: LCDDATA write ignored!\n";
			}
			...
			uint16_t effective_addr = (static_cast<uint16_t>(current_bsr_val & 63) << 7) | (current_ptr_val & 0x7F);
			if (effective_addr < sizeof(ram)) {
				ram[effective_addr] = static_cast<char>(value_to_write);
			}
			else {
				std::cerr << "Warning: WriteDat out of RAM bounds: 0x" << std::hex << effective_addr << std::endl;
			}
		}

		// Post-ID logic (same as ReadDat)
		if (is_indirect) {
			if (original_rptr_for_post_id == SFRs::INDF0) {
				if (getFSR0_PE()) {
					uint8_t temp_fsr0 = static_cast<uint8_t>(regs[SFRs::FSR0]);
	...
	void execute_one_instruction() {
		...
		case 0x2F: // gh = 11 (ACC indexed)
		{
			...
			byte_from_rom = static_cast<uint8_t>(rom[tabptr_val]);

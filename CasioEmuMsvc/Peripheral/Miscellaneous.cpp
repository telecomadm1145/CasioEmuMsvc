...
namespace casioemu {
	...
	void Miscellaneous::Initialise() {
		region_dsr.Setup(
			0xF000, 1, "Miscellaneous/DSR", this, [](MMURegion* region, size_t) { return (uint8_t)((Miscellaneous*)region->userdata)->emulator.chipset.cpu.impl_last_dsr; }, [](MMURegion* region, size_t, uint8_t data) { ((Miscellaneous*)region->userdata)->emulator.chipset.cpu.impl_last_dsr = data; }, emulator);
		...
		if (emulator.hardware_id == HW_5800P) {
			region_F046.Setup(
				0xF046, 1, "F046", 0, [](MMURegion*, size_t) -> uint8_t { return 4; }, [](MMURegion*, size_t, uint8_t) { 4; }, emulator);
		}
		...
		if (emulator.hardware_id == HW_CLASSWIZ_II) {
			...
			region_F0D1.Setup(
				0xF0D1, 1, "Miscellaneous/Battery/F0D1", this, [](MMURegion* region, size_t offset) {
				Miscellaneous* miscellaneous = (Miscellaneous*) region->userdata;
				return miscellaneous->data_F0D1; }, [](MMURegion* region, size_t offset, uint8_t data) {
				Miscellaneous* miscellaneous = (Miscellaneous*) region->userdata;
				if(miscellaneous->data_F0D0 == 3 && miscellaneous->data_F0D2 == 0 && data == 5) {
					miscellaneous->data_F0D1 = 6;
					return;
				}
				miscellaneous->data_F0D1 = data; }, emulator);
		}
		if (emulator.hardware_id == HW_5800P) {
			// static MMURegion region_f009;
		}
	}
}

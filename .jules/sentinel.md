...
## 2024-05-10 - Replace `sprintf` with bounds-checked `snprintf` for ImGui buffers
**Vulnerability:** Uses of `sprintf` without bounds-checking in ImGui text and format operations on fixed-size buffers like `char buf[32]` could cause stack-based buffer overflows if format specifiers combined with user input expanded beyond the buffer size.
**Learning:** ImGui text buffers in this codebase are generally implemented as static fixed-size character arrays (e.g., `char buf[32]`). It's always safer to write to them with length-constrained formatting rather than assuming input strings will conform to a set size.
**Prevention:** Replace instances of `sprintf` with bounds-checked alternatives like `snprintf`, using `sizeof()` on array buffers.

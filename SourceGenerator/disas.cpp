...
// ------------------------------------------------------------------ 1
// split version: display 4-byte commands as two 2-byte commands.
...
// mmm01000 1010nnn0 L       ER{n*2}, simm16[ER{m*2}]
if ((buf[0] & 0b00011111) == 0b00001000 && (buf[1] & 0b11110001) == 0b10100000) {
	int m = buf[0] >> 5 & 0b111, n = buf[1] >> 1 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "L       ER" << (n*2) << ", simm16[ER" << (m*2) << "]"
		<< "\n";
    buf.pop_front(); buf.pop_front();
	goto done;
}
// 00010010 1001nnn0 L       ER{n*2}, imm16
if ((buf[0] & 0b11111111) == 0b00010010 && (buf[1] & 0b11110001) == 0b10010000) {
	int n = buf[1] >> 1 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "L       ER" << (n*2) << ", imm16"
		<< "\n";
...
// mmm01000 1001nnnn L       R{n}, simm16[ER{m*2}]
if ((buf[0] & 0b00011111) == 0b00001000 && (buf[1] & 0b11110000) == 0b10010000) {
	int m = buf[0] >> 5 & 0b111, n = buf[1] >> 0 & 0b1111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "L       R" << (n) << ", simm16[ER" << (m*2) << "]"
		<< "\n";
...
// 00010000 1001nnnn L       R{n}, imm16
if ((buf[0] & 0b11111111) == 0b00010000 && (buf[1] & 0b11110000) == 0b10010000) {
	int n = buf[1] >> 0 & 0b1111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "L       R" << (n) << ", imm16"
		<< "\n";
...
// mmm01001 1010nnn0 ST      ER{n*2}, simm16[ER{m*2}]
if ((buf[0] & 0b00011111) == 0b00001001 && (buf[1] & 0b11110001) == 0b10100000) {
	int m = buf[0] >> 5 & 0b111, n = buf[1] >> 1 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "ST      ER" << (n*2) << ", simm16[ER" << (m*2) << "]"
		<< "\n";
    buf.pop_front(); buf.pop_front();
	goto done;
}
// 00010011 1001nnn0 ST      ER{n*2}, imm16
if ((buf[0] & 0b11111111) == 0b00010011 && (buf[1] & 0b11110001) == 0b10010000) {
	int n = buf[1] >> 1 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "ST      ER" << (n*2) << ", imm16"
		<< "\n";
...
// mmm01001 1001nnnn ST      R{n}, simm16[ER{m*2}]
if ((buf[0] & 0b00011111) == 0b00001001 && (buf[1] & 0b11110000) == 0b10010000) {
	int m = buf[0] >> 5 & 0b111, n = buf[1] >> 0 & 0b1111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "ST      R" << (n) << ", simm16[ER" << (m*2) << "]"
		<< "\n";
    buf.pop_front(); buf.pop_front();
	goto done;
}
// 00010001 1001nnnn ST      R{n}, imm16
if ((buf[0] & 0b11111111) == 0b00010001 && (buf[1] & 0b11110000) == 0b10010000) {
	int n = buf[1] >> 0 & 0b1111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "ST      R" << (n) << ", imm16"
		<< "\n";
...
// mmm01011 11110000 LEA     simm16[ER{m*2}]
if ((buf[0] & 0b00011111) == 0b00001011 && (buf[1] & 0b11111111) == 0b11110000) {
	int m = buf[0] >> 5 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "LEA     simm16[ER" << (m*2) << "]"
		<< "\n";
...
// 00001100 11110000 LEA     imm16
if ((buf[0] & 0b11111111) == 0b00001100 && (buf[1] & 0b11111111) == 0b11110000) {
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "LEA     imm16"
		<< "\n";
    buf.pop_front(); buf.pop_front();
...
// 1bbb0000 10100000 SB      imm16.{b}
if ((buf[0] & 0b10001111) == 0b10000000 && (buf[1] & 0b11111111) == 0b10100000) {
	int b = buf[0] >> 4 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "SB      imm16." << (b) << ""
		<< "\n";
...
// 1bbb0010 10100000 RB      imm16.{b}
if ((buf[0] & 0b10001111) == 0b10000010 && (buf[1] & 0b11111111) == 0b10100000) {
	int b = buf[0] >> 4 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "RB      imm16." << (b) << ""
		<< "\n";
...
// 1bbb0001 10100000 TB      imm16.{b}
if ((buf[0] & 0b10001111) == 0b10000001 && (buf[1] & 0b11111111) == 0b10100000) {
	int b = buf[0] >> 4 & 0b111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "TB      imm16." << (b) << ""
		<< "\n";
...
// 00000000 1111gggg B       0{tohex(g, 1)}h:cadr_imm16
if ((buf[0] & 0b11111111) == 0b00000000 && (buf[1] & 0b11110000) == 0b11110000) {
	int g = buf[1] >> 0 & 0b1111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "B       0" << (tohex(g, 1)) << "h:cadr_imm16"
		<< "\n";
    buf.pop_front(); buf.pop_front();
	goto done;
}
// 00000001 1111gggg BL      0{tohex(g, 1)}h:cadr_imm16
if ((buf[0] & 0b11111111) == 0b00000001 && (buf[1] & 0b11110000) == 0b11110000) {
	int g = buf[1] >> 0 & 0b1111;
	out << tohex(ip, 6) << "   " << tohex(buf[0], 2) << ' ' << tohex(buf[1], 2) << ' ' << "             "
		<< "BL      0" << (tohex(g, 1)) << "h:cadr_imm16"
		<< "\n";
    buf.pop_front(); buf.pop_front();
	goto done;
}
